...
app.post('/admin/log-hours', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const newProjectsHours = parseFloat(hours_worked);
        ...
        let existingHours = 0;
        if (existingEntry) {
            existingHours = existingEntry.hours_worked || 0;
        }

        // Check if adding new hours would exceed 14-hour limit
        const totalHoursAfterAddition = existingHours + newProjectsHours;

        if (totalHoursAfterAddition > 14) {
            const remainingHours = 14 - existingHours;
            console.error(`? Admin logging: Daily hour limit exceeded for ${employee.name}. Current: ${existingHours}h, Attempting to add: ${newProjectsHours}h`);
            
            if (remainingHours <= 0) {
                return res.status(400).send(`Cannot log more hours for ${date}. ${employee.name} has already logged the maximum of 14 hours for this date.`);
            } else {
                return res.status(400).send(`Cannot log ${newProjectsHours} hours for ${date}. ${employee.name} can only log ${remainingHours} more hours (Current: ${existingHours}h, Limit: 14h per day).`);
            }
        }
        ...
        if (existingEntry) {
            // Append projects to existing entry
...
app.post("/log-hours", isAuthenticated, async (req, res) => {
    try {
        ...
        // Process new projects being added
        let newProjects = [];
        if (Array.isArray(project_name)) {
            newProjects = project_name.map((name, index) => ({
                name,
        ...
        console.log("DEBUG: New Projects Being Added:", JSON.stringify(newProjects, null, 2));

        // Calculate total hours for new projects
        const newProjectsHours = newProjects.reduce((sum, proj) => sum + proj.hours, 0);

        // Get existing hours for this date
        let existingHours = 0;
        if (workHourEntry) {
            existingHours = workHourEntry.hours_worked || 0;
        }

        // Calculate total hours if we add these new projects
        const totalHoursAfterAddition = existingHours + newProjectsHours;

        // Check if total would exceed 14 hours
        if (totalHoursAfterAddition > 14) {
            const remainingHours = 14 - existingHours;
            console.error(`? Daily hour limit exceeded. Current: ${existingHours}h, Attempting to add: ${newProjectsHours}h, Limit: 14h`);
            
            if (remainingHours <= 0) {
                return res.status(400).send(`Cannot log more hours for ${logDate.toISODate()}. You have already logged the maximum of 14 hours for this date.`);
            } else {
                return res.status(400).send(`Cannot log ${newProjectsHours} hours for ${logDate.toISODate()}. You can only log ${remainingHours} more hours (Current: ${existingHours}h, Limit: 14h per day).`);
            }
        }

        const combinedDescription = newProjects.map(proj => `${proj.name}: ${proj.description}`).join("; ");
        ...
        if (workHourEntry) {
            ...
            await WorkHour.create({
                ...
                hours_worked: newProjects.reduce((sum, proj) => sum + proj.hours, 0),
                description: combinedDescription,
                signature: signaturePath
            });
